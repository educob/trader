    ...
    try { // server: chromedriver
      ...
      for(const url of urls) {
        ...
        for(const li of lis) {
          ...
          if(!!found) {
            console.warn("News already processed:", href)
            break
          }
          ...
          if(!!firstNews)  {
            ...
            console.warn("updating last_news for ", url)
            firstNews = false
          }
          ...
          const news_url = `finance.yahoo.com${href}`
            ...
            let analysis = await gpt.analyze_news(company, news_text)
            ...
            if(!analysis || ['positive', 'negative'].includes(analysis.impact)) {
              if(!analysis) 
                analysis.impact = 'No GPT'
              companies_w_news.push( { href: news_url, impact: analysis.impact  } )
              console.warn(`news:, ${news_url}. Impact: ${analysis.impact}`)
            }
