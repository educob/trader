...
const pattern = {
  ...
  async detect_patterns(symbols) { //console.log("detect_patterns symbol:", symbol)
    ...
    for(let i = 0; i < symbols.length; i++) {
      ...
      // 50 sma distance
      sma50Distances.push( { symbol, distance: utils.sma50Distance(db_bars) } )
      ATRs.push( { symbol, atr: utils.atr(db_bars) } )
